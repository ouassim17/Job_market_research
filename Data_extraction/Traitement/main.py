    ...
    for py_file in py_files:
        ...
        try:
            result = subprocess.run(
                ["python", file_path], check=True, capture_output=True, text=True
            )
            if result.stdout:
                ...
                if match:
                    ...
                    logging.warning(
                        f"Aucun nombre d'offres trouvé dans la sortie de {py_file}."
                    )

        except subprocess.CalledProcessError as e:
            logging.error(
                f"Erreur lors de l'exécution de {py_file}: Code {e.returncode} - {e.stderr}"
            )
